...
const Vacancy = document.getElementById('vacancy');  //Vacancy value for which the user has applied.
...
const Messageinvalid = document.getElementById('messageinvalid');
const Vacancyvalid = document.getElementById('vacancyvalid');
const Vacancyinvalid = document.getElementById('vacancyinvalid');
...
Vacancyvalid.style.display = 'none';
Vacancyinvalid.style.display = 'none';
...
Email.addEventListener('blur', () => {
    ...
    if (Test) {
        ...
        Emailinvalid.style.display = 'none';
        validEmail = true; //valid Email variable will se changed to true.
    } else {
...
Phone.addEventListener('blur', () => {
    //Checking the phone of the user based on the Regex and if it met the condition then the success message will appear or failure message will appear.
    ...
    if (Test) {
        ...
        Phoneinvalid.style.display = 'none';
        validPhone = true; //valid Phone variable will se changed to true.
    } else {
...
Vacancy.addEventListener('change', () => {
    //Checking the phone of the user based on the Regex and if it met the condition then the success message will appear or failure message will appear..
    let VacancyData = Vacancy.value; //value inside the phone input box.
    if (VacancyData != "-1") {
        //if the test value will be true then the below class will get added and removed from the vacancyvalid and vacancyinvalid div.
        Vacancyvalid.style.display = 'block';
        Vacancyinvalid.style.display = 'none';
        validVacancy = true; //valid Vacancy variable will se changed to true.
    } else {
        //if the test value will be false then the below class will get added and removed from the validvacancy and invalidvacancy div.
        Vacancyvalid.style.display = 'none';
        Vacancyinvalid.style.display = 'block';
    };
});
...
Message.addEventListener('blur', () => {
    ...
    if (Test) {
        ...
        Messageinvalid.style.display = 'none';
        validMessage = true; //valid message variable will se changed to true.
    } else {
...
SubmitButton.addEventListener('click', (event) => {
    ...
    if (validName && validEmail && validPhone && validMessage && validVacancy) {
        ...
        fetch('/carrierdata/', options). //sending the request to the 'get-response/' end point of the backend.
            then(res => res.json()). //converting the the response into json format.
            then((res) => {
                if (res.success == true) {
                    Form.reset();  //this is used to reset the form.
                    SuccessAlert.style.display = 'block';  //this will show the success message.
                    FailureAlert.style.display = 'none';  //this will hide the failure message.
                    Refresh(); //it will refresh the page.
                }
                else if (res.success == false) {
                    Form.reset();  //this is used to reset the form.
                    FailureAlert.style.display = 'block';  //this will hide the success message.
                    SuccessAlert.style.display = 'none'; //this will hide the success message.
                    Refresh(); //it will refresh the page.
                }
            }).
            ...
            //if any error occured it will show in message.
                ...
                Form.reset();  //this is used to reset the form.
                FailureAlert.style.display = 'block';  //this will show the failure message.
                SuccessAlert.style.display = 'none';  //this will hide the success message.
                Refresh(); //it will refresh the page.
            }
            );
